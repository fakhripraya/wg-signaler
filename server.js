...
io.on("connection", (socket) => {
    socket.on("join room", (userJoin, roomCode) => {
        ...
        const otherUser = rooms[roomCode].users.find(
            (user) => user.id !== userJoin.id
        );
        if (otherUser) {
    ...
    socket.on("disconnecting", () => {
        ...
        const userSocket = rooms[socket.joinedRoom].users.find(
...
app.get("/v1/room/check", (req, res) => {
    ...
    if (rooms[query.roomCode].users.find((user) => user.id === query.user.id))
        return res
            .send(
                JSON.stringify({
    ...
    if (rooms[query.roomCode].users.length >= 2)
        return res
            .send(
                JSON.stringify({
                    code: ROOM_FULL,
